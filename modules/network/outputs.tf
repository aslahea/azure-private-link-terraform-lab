...
output "vnet1_peering_name" {
  ...
  value       = local.vnet2_key != null ? azurerm_virtual_network_peering.this["${local.vnet1_key}_to_${local.vnet2_key}"].name : null
}

output "vnet2_peering_name" {
  description = "Name of peering from VNet 2 to VNet 1"
  value       = local.vnet2_key != null ? azurerm_virtual_network_peering.this["${local.vnet2_key}_to_${local.vnet1_key}"].name : null
}
